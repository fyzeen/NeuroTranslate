...
if __name__ == "__main__":
    ...
    #n=200
    #train_data_np = train_data_np[:2, :, :, :]
    #train_label_np = train_label_np[:2, :]

    #test_data_np = test_data_np[:2, :, :, :]
    #test_label_np = test_label_np[:2, :]
    ...
    write_fpath = "/home/ahmadf/batch/sbatch.printTestGraph"
    ...
    with torch.no_grad():
        for i, data in enumerate(train_loader):
            ...
            decoder_pred = NEW_greedy_decode_graph(model=model, source=inputs, latent_length=model.input_dim, device=device, b=batch_size)

            '''# just take lower triangle (most attention-aware)
            pred_mat = decoder_pred.squeeze().detach().numpy()[2:, 2:]
            truth_mat = targets.squeeze().numpy()[2:, 2:]
            write_to_file(np.mean(np.abs(truth_mat[np.triu_indices(len(pred_mat), k=1)] - pred_mat[np.triu_indices(len(pred_mat), k=1)])), write_fpath)
            write_to_file(np.corrcoef(truth_mat[np.triu_indices(len(pred_mat), k=1)], pred_mat[np.triu_indices(len(pred_mat), k=1)]), write_fpath)
            write_to_file("###", write_fpath)'''
        ...
        for i, data in enumerate(test_loader):
            ...
            #write_to_file(np.mean(np.abs(targets.squeeze().numpy()[pad:] - convDecoder_pred.squeeze().detach().numpy()[pad:])), filepath=write_fpath)
            test_maes.append(np.mean(np.abs(targets.squeeze().numpy()[2:, 2:] - decoder_pred.squeeze().detach().numpy()[2:, 2:])))
