...
def test(model, train_loader_fortesting, test_loader, device, mean_train_label):
    model.eval()
    model.to(device)
    ...
    with torch.no_grad():
        for i, data in enumerate(train_loader_fortesting):
            ...
            corr = np.corrcoef(targets.squeeze().numpy() - mean_train_label, convDecoder_pred.squeeze(0) - mean_train_label)[0, 1]
...
if __name__ == "__main__":
    ...
    model_type = "KrakLossConvTransformer_PenultimateLayerKrakloss"
    out_nodes = 100
    ...
    for epoch in range(0, 601):
        ...
        if epoch%10 == 0:
            train_mse, train_mae, test_mse, test_mae, train_corr, test_corr = test(model, train_loader_fortesting, test_loader, device, mean_train_label)
