...
def train(model, train_loader, device, input_dim, optimizer, epoch, reset_params=True):
    ...
    for i, data in enumerate(train_loader):
        ...
        log_var = torch.clamp(log_var, max = 10.0) # clamping logvar -- IS THIS ALRIGHT -- @SHIRIN

        kld_loss = torch.mean(-0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim = 1), dim = 0)
        recon_loss = torch.nn.MSELoss()(pred, targets)

        # Output Losses
        Lr_corrI = correye(targets, pred) # corr mat of measured->predicted should be high along diagonal, low off diagonal 
        ...
        #Lz_dist = distance_loss(latent, latent, neighbor=False) / (batch_size**2) # mean intersubject altent space distances should be high -- COMMENTED BC THIS GIVES ISSUES
...
if __name__ == "__main__":
    ...
    model_type = "PCAVariationalKrakLossConvTransformer_LogvarClamp"
    out_nodes = 100
